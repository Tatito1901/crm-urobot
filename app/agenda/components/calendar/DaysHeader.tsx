...
export const DaysHeader = React.memo(function DaysHeader({ weekStart, mode = 'week' }: DaysHeaderProps) {
  ...
  return (
    <div
      ...
      } bg-background sticky top-0 z-20 border-b border-border transition-colors`}
    >
      {/* Columna vacía para alinear con columna de horas */}
      <div className="bg-background z-20 border-r border-border transition-colors" />
      ...
      {days.map((date, index) => {
        ...
        return (
          <div
            key={index}
            className={`relative flex flex-col items-center justify-center py-4 border-r border-border last:border-r-0 group transition-colors`}
            onMouseEnter={() => setHoveredDay(index)}
            onMouseLeave={() => setHoveredDay(null)}
          >
            <span className={`text-[11px] font-medium mb-1 uppercase tracking-wider ${isTodayDate ? 'text-primary' : 'text-muted-foreground'}`}>
              {dayName}
            </span>
            
            {isTodayDate ? (
              <div className="flex items-center justify-center w-10 h-10 rounded-full bg-primary text-primary-foreground shadow-lg shadow-primary/20">
                <span className="text-2xl font-normal leading-none pt-[2px]">
                  {dayNumber}
                </span>
              </div>
            ) : (
              <div className="flex items-center justify-center w-10 h-10 rounded-full hover:bg-accent transition-colors text-foreground">
                <span className="text-2xl font-normal leading-none pt-[2px]">
                  {dayNumber}
                </span>
              </div>
