...
const CustomTooltip = React.memo(({ active, payload }: CustomTooltipProps) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-900/95 backdrop-blur-sm border border-slate-700/50 rounded-md px-3 py-2 shadow-xl">
...
export const BarChart: React.FC<BarChartProps> = React.memo(({ data, height = 200 }) => {
  ...
  if (!hasData) {
    return (
      <div
        className="flex items-center justify-center w-full bg-slate-800/20 rounded-lg border border-slate-800/50 border-dashed"
        style={{ height }}
      >
