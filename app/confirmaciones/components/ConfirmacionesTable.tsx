...
export const ConfirmacionesTable = React.memo(function ConfirmacionesTable({ 
  ...
  const rows = useMemo(() => recordatorios.map((recordatorio) => ({
    ...
    consulta: (
      <span className="font-mono text-[10px] text-slate-400 bg-slate-800/50 px-1.5 py-0.5 rounded">
    ...
    canal: (
      <div className="flex justify-center sm:justify-start">
        <Badge label={recordatorio.canal || 'whatsapp'} variant="outline" className="border-slate-700 text-slate-400" />
      </div>
    ),
  })), [recordatorios]);
